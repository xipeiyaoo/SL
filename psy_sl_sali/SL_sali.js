...
let expName = 'SL_sali';  // from the Builder filename that created this script
...
var green;
var red;
var circle_sali_vertices;
var a;
var diamond_vertices;
var c;
var diamond_sali_vertices;
var b;
var hori_start;
var hori_end;
var vert_start;
...
async function experimentInit() {
  ...
  x = [];
  y = [];
  green =  new util.Color('red');
  red = new util.Color('green');
  ...
  distractor_locs = [];
  tar_only_locs = [];
  for (var _, _pj_c = 0, _pj_a = util.range(24), _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
      _ = _pj_a[_pj_c];
  ...
  for (var i, _pj_c = 0, _pj_a = util.range(120), _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
      i = _pj_a[_pj_c];
      new_trial = [];
...
function formal_cueRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of formal_cueComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
